...
> **Status:** early reverse-engineering / development project. The currently proven protocol baseline is intentionally kept separate from the future Home Assistant layer.

## Goals

- Run the robot without a live RobotBona/Proscenic cloud connection.
- Keep protocol knowledge, capabilities, state interpretation and map decoding in a reusable core/server — **not inside the Home Assistant integration**.
...
The Home Assistant integration is deliberately a presentation/control adapter. It must not become the authoritative implementation of the proprietary robot protocol or the only place where robot capabilities and state semantics are defined.
...
| `1`, `8`, `10`, `11` | command accepted, but no distinct mode observed on this firmware | not exposed as confirmed modes |
...
src/             reusable RobotBona core/server
docs/            protocol and reverse-engineering notes
tests/           regression tests and sanitized fixtures
homeassistant/   Home Assistant custom integration (thin client)
ha-app/          Home Assistant App packaging
```

## Privacy / public-repository policy

Do **not** commit real device identifiers, MAC addresses, auth codes, Wi-Fi credentials, private IP assignments, packet captures, raw proxy logs, or captured cloud/local tokens. Use sanitized fixtures and configuration/environment variables instead.
...
Historical Home Assistant work in [`deblockt/hass-proscenic-790T-vacuum`](https://github.com/deblockt/hass-proscenic-790T-vacuum) was useful as a behavioural reference. Protocol facts are independently verified where marked as confirmed; code from that repository is not copied here.
